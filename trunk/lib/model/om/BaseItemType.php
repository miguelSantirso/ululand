...
abstract class BaseItemType extends BaseObject  implements Persistent {
    ...
    foreach (sfMixer::getCallables('BaseItemType:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseItemType:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseItemType:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseItemType:save:post') as $callable)
    {
		...
		if ($this->collItems === null) {
			if ($this->isNew()) {
			   $this->collItems = array();
				...
				$criteria->add(ItemPeer::ID_ITEMTYPE, $this->getId());
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseItemType:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseItemType::%s', $method));
    }
