...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	...
	protected $collCollaborationOffers;
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach($this->getCollaborationOffers() as $relObj) {
	...
	public function initCollaborationOffers()
	{
		if ($this->collCollaborationOffers === null) {
			$this->collCollaborationOffers = array();
		}
	}

	
	public function getCollaborationOffers($criteria = null, $con = null)
	{
				include_once 'lib/model/om/BaseCollaborationOfferPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		}
		elseif ($criteria instanceof Criteria)
		...
		if ($this->collCollaborationOffers === null) {
			if ($this->isNew()) {
			   $this->collCollaborationOffers = array();
				...
				$criteria->add(CollaborationOfferPeer::CREATED_BY, $this->getId());

				CollaborationOfferPeer::addSelectColumns($criteria);
				$this->collCollaborationOffers = CollaborationOfferPeer::doSelect($criteria, $con);
			}
		} else {
						if (!$this->isNew()) {
				...
				CollaborationOfferPeer::addSelectColumns($criteria);
				if (!isset($this->lastCollaborationOfferCriteria) || !$this->lastCollaborationOfferCriteria->equals($criteria)) {
					$this->collCollaborationOffers = CollaborationOfferPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastCollaborationOfferCriteria = $criteria;
		return $this->collCollaborationOffers;
	}

	
	public function countCollaborationOffers($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/om/BaseCollaborationOfferPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		return CollaborationOfferPeer::doCount($criteria, $distinct, $con);
	}

	
	public function addCollaborationOffer(CollaborationOffer $l)
	{
