...
public class b2PolygonShape extends b2Shape
{
	...
	public override function TestPoint(xf:b2XForm, p:b2Vec2) : Boolean{
		...
		for (var i:int = 0; i < m_vertexCount; ++i)
		{
			...
			var dot:Number = (m_normals[i].x * tX + m_normals[i].y * tY);
			if (dot > 0.0)
			{
				return false;
			}
		...
		transform2:b2XForm) : void
	{
		...
		aabb.lowerBound.Set(Math.min(aabb1.lowerBound.x, aabb2.lowerBound.x),
							Math.min(aabb1.lowerBound.y, aabb2.lowerBound.y));
		//aabb.upperBound = b2Max(aabb1.upperBound, aabb2.upperBound);
		aabb.upperBound.Set(Math.max(aabb1.upperBound.x, aabb2.upperBound.x),
							Math.max(aabb1.upperBound.y, aabb2.upperBound.y));
	}
	...
	public override function UpdateSweepRadius(center:b2Vec2) : void{
		...
		for (var i:int = 0; i < m_vertexCount; ++i)
		{
			...
			var dX:Number = m_coreVertices[i].x - center.x;
			var dY:Number = m_coreVertices[i].y - center.y;
