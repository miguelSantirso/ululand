...
static public function ProcessThree(x1:b2Vec2, x2:b2Vec2, p1s:Array, p2s:Array, points:Array):int
{
	...
	//b2Vec2 c = points[2];
	var cX:Number = points[2].x;
	var cY:Number = points[2].y;
...
static public function InPoints(w:b2Vec2, points:Array, pointCount:int):Boolean
{
	...
	for (var i:int = 0; i < pointCount; ++i)
	{
		...
		var dX:Number = Math.abs(w.x - points[i].x);
		var dY:Number = Math.abs(w.y - points[i].y);
		//b2Vec2 m = b2Max(b2Abs(w), b2Abs(points[i]));
		var mX:Number = Math.max(Math.abs(w.x), Math.abs(points[i].x));
		var mY:Number = Math.max(Math.abs(w.y), Math.abs(points[i].y));
